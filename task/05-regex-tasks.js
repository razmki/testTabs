...
function getRegexForGuid() {
  return /\{[A-Fa-f0-9_]{8}-[A-Fa-f0-9_]{4}-[A-Fa-f0-9_]{4}-[A-Fa-f0-9_]{4}-[A-Fa-f0-9_]{12}\}/;
}
...
function getRegexForSSN() {
  return /(?!000)[0-9_]{3}-(?!00)[0-9_]{2}-(?!0000)[0-9_]{4}/
}
...
function getPasswordValidator(minLength) {
  return new RegExp('(?=^.{' + minLength + ',}$)((?=.*\\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z])[a-zA-Z0-9]{' + minLength + ',}.*$');
}
