...
function removeLeadingAndTrailingWhitespaces(value) {
	return value.replace(/^\s{1,}|\s{2,}|\s{1,}$/g, ''); 
}
...
function encodeToRot13(str) {
    ...
    for(var i = 0; i < str.length; i++) {
        if (str.charCodeAt(i) > 122 || str.charCodeAt(i) < 64) {
            res += str[i];
        } else {
            if (str[i].toUpperCase() === str[i]) {
                var lowCase = str[i].toLowerCase();
                var temp = lowCase.charCodeAt(0) + 13;
                res += (temp > 122)? String.fromCharCode(temp - 122 + 96).toUpperCase() : String.fromCharCode(temp).toUpperCase();
            } else {
                var temp = str.charCodeAt(i) + 13;
                res += (temp > 122)? String.fromCharCode(temp - 122 + 96) : String.fromCharCode(temp);
            }
        }
    }
    return res;
